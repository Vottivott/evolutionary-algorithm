...
class Ball(Circular):
    def __init__(self, position, radius, friction):
    ...
    def bounce_on_level(self, level):
        ...
        if bounce_direction is not None:
            ...
            self.position += 0.01*-vel_component_in_bounce_direction * bounce_direction
            i = 0
            while level.collides_with_circular(self) and i < 40:
                self.position += 0.01*-vel_component_in_bounce_direction * bounce_direction
                i+=1
